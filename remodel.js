(()=>{'use strict';
 const fieldScript=document.createElement('script');fieldScript.src='/linsa-rpg/field-visuals.js?v=W2-1';document.head.append(fieldScript);
window.installRinguRemodel=function(g){
 ...
 // Abyss rewards are final amounts, not inputs to the legacy reward multipliers.
 g.bossRegions[5].bosses.forEach((b,i)=>{b.reward=[6875,10080,13965,18560,23895,30000][i];});
 ...
 function gearTitle(it,showProgress=true){if(window.RinguWorld2?.template(it))return f.itemTitleHtml(it,showProgress);const count=f.gearVariantCount(Number(it.rarity),it.slot),base=f.fixedBaseAtk(it.slot,Number(it.rarity),f.itemIndex(it)),place=1+Array.from({length:count},(_,i)=>f.fixedBaseAtk(it.slot,Number(it.rarity),i)).filter(v=>v>base).length,rank=window.RinguGearSets?.rank(it)||{rank:place,roman:['I','II','III','IV','V','VI','VII','VIII','IX','X'][place-1]},stars='★'.repeat(Math.max(0,Math.min(3,Number(it.transcend)||0))),color=g.rarityColors[it.rarity]||'#dce8ff';return '<span class="cube-item-name">'+esc((it.locked?'[잠금] ':'')+(window.RinguAngel?.displayName(it)||it.name))+'</span>'+(rank?' <span class="rm-name-rank" title="등급 내 세트 '+rank.rank+'위" style="font-size:.8em;flex-shrink:0;white-space:nowrap;margin-left:.3em">'+rank.roman+'</span>':'')+(showProgress?' <span class="ringu-plus">+'+(Number(it.enhance)||0)+'</span>':'')+(showProgress&&stars?' <span class="transcend-stars" style="color:'+color+';--star-color:'+color+'" aria-label="'+(Number(it.transcend)||0)+'초월">'+stars+'</span>':'');}
 f.itemTitleHtml=gearTitle;
 f.gearIcon=it=>art.icon(it,f.itemIndex(it));f.ringuResultIcon=f.gearIcon;
 window.openItemDetail=id=>{const it=state().inventory.find(item=>String(item.id)===String(id));if(!it)return;let modal=$('rmItemDetail');if(!modal){modal=document.createElement('div');modal.id='rmItemDetail';modal.className='modal-bg';modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');modal.setAttribute('aria-label','장비 상세');document.body.append(modal);}modal.innerHTML='<section class="modal"><h3>'+gearTitle(it)+'</h3><div class="rm-detail-art">'+f.gearIcon(it)+'</div><p>'+g.rarityNames[it.rarity]+' · '+esc(it.slot)+' · '+(f.isItemEquipped(it)?'✓ 장착 중':'미장착')+'</p><h3>⚔ '+f.itemAttackText(it)+'</h3><p>'+f.optionText(it)+'</p><p>강화 +'+it.enhance+'　초월 '+(it.transcend||0)+'　'+(it.locked?'🔒 잠금':'')+'</p><button id="rmDetailEnhance">강화 / 초월</button><button id="rmDetailClose">닫기</button></section>';$('rmDetailEnhance').onclick=()=>{modal.classList.remove('show');f.openEnhance(it.id)};$('rmDetailClose').onclick=()=>modal.classList.remove('show');modal.classList.add('show');$('rmDetailClose').focus();};
 f.sortInventoryItems=old.sortInventoryItems;
 f.renderEquipment=()=>{const attack=fmt(f.getPlayerStats().attack);$('equipAtk').hidden=true;const current=$('rmPortraitAttack');if(current)current.textContent='⚔ 공격력 '+attack;const s=state(),map=equipMap(),signature=JSON.stringify([s.equipped,s.inventory.filter(it=>map[it.slot]===it).map(it=>[it.id,it.enhance,it.transcend]),s.playerGender,s.playerName,s.equippedAura,s.equippedPet,s.ownedPets?.find(p=>p.uid===s.equippedPet)?.level]);if($('equipGrid').dataset.signature===signature)return;$('equipGrid').dataset.signature=signature;
  $('equipGrid').innerHTML='<div class="rm-character-stage"><div id="rmPortraitAttack" class="rm-portrait-attack" aria-live="polite">⚔ 공격력 '+attack+'</div><canvas id="heroCanvas" width="640" height="780" aria-label="장비 외형과 독립된 캐릭터"></canvas><div class="rm-character-caption"><small>ADVENTURER</small><strong>'+esc(s.playerName)+'</strong><span>외형은 장비와 독립 · 전투 시 무기 표시</span></div></div>'+RinguGearSets.html(Object.values(map))+'<div class="rm-equipped-list">'+g.slots.map(slot=>{const it=map[slot];return '<button class="rm-equipped-slot" '+(it?'onclick="openEnhance('+it.id+')"':'onclick="scrollToId(\'inventoryPanel\')"')+' style="--rarity:'+(it?g.rarityColors[it.rarity]:'#697178')+'">'+(it?f.gearIcon(it):'<span class="rm-empty-slot">＋</span>')+'<span><small>'+slot+'</small><strong>'+(it?gearTitle(it):'미장착')+'</strong><em>'+(it?'⚔ '+f.itemAttackText(it):'가방에서 장비 선택')+'</em></span></button>'}).join('')+'</div>';$('equipAtk').textContent='⚔ '+fmt(f.getPower());
 };
 const monsterPreviews=new Map();function monsterPreview(i){const frame=art.monsterFrames[i];if(!frame)return '';if(!monsterPreviews.has(i))monsterPreviews.set(i,frame.toDataURL('image/png'));return monsterPreviews.get(i);}
 f.renderBosses=()=>{const s=state();$('regionTabs').innerHTML=g.bossRegions.map((r,i)=>'<button class="region-tab '+(i===s.regionIndex?'active':'')+'" onclick="selectRegion('+i+')">'+['초원','사막','설원','화산','천공','심연'][i]+'</button>').join('');$('bossList').innerHTML=f.currentBosses().map((b,i)=>'<button class="boss-btn '+(i===s.bossIndex?'active':'')+'" onclick="selectBoss('+i+')"><img class="rm-monster-icon" alt="" src="'+monsterPreview(s.regionIndex*6+i)+'"><strong>'+esc(b.name)+'</strong><span>♥ '+fmt(b.hp)+'　◈ '+fmt(b.reward)+'</span></button>').join('');};
 ...
 f.renderAll=(options)=>{if(!state())return;f.renderTop();f.renderBosses();f.renderBattle();f.renderEquipment();f.renderPlayerStats();f.renderSummon();if(options?.inventory!==false)f.renderInventory();window.renderMailboxBadge?.();f.renderDailyRewardBoxV1();paintPetIcons();};
 f.renderPetLayer=()=>'';
 ...
 let operation=null;
 f.openEnhance=id=>{if(operation)return f.toast('강화가 끝난 뒤 다른 장비를 선택하세요.');$('enhanceStage').className='enhance-stage';$('enhanceResult').textContent='';g.enhanceId=state().inventory.find(it=>String(it.id)===String(id))?.id??null;if(g.enhanceId===null)return;f.renderEnhance();$('enhanceModal').classList.add('show')};
 ...
 f.renderEnhance=()=>{const it=state().inventory.find(i=>String(i.id)===String(g.enhanceId));if(!it)return;const next=(it.transcend||0)+1;$('enhanceTitle').innerHTML=gearTitle(it);$('enhanceGear').innerHTML=f.gearIcon(it);$('enhanceDesc').innerHTML='⚔ '+f.itemAttackText(it)+'　'+f.optionText(it);$('enhNow').textContent='+'+it.enhance+'　'+'★'.repeat(it.transcend||0);$('enhRate').textContent=it.enhance>=15?'MAX':f.successRate(it.enhance+1)+'%';$('enhCost').textContent=it.enhance>=15?'—':fmt(f.enhanceCost(it))+' G';$('enhanceBtn').disabled=!!operation||it.enhance>=15;$('enhanceBtn').textContent=operation?'진행 중…':'강화 시도';$('transcendBtn').disabled=!!operation||!f.canTranscend(it);$('transcendBtn').textContent=(it.transcend||0)>=3?'최대 초월 달성':next+'초월 시도';$('transcendInfo').textContent=f.canTranscend(it)?'초월석 '+f.transcendCost(it,next)+'개 · 성공률 '+f.transcendRate(next)+'%':'전설 이상, +15 강화 장비만 초월할 수 있습니다.';$('protectToggle').textContent='하락 방지 '+(state().useProtect?'ON':'OFF')+' · 보유 '+state().downgradeProtect+'장';};
 function forge(transcend){if(operation||!active())return false;const it=state().inventory.find(i=>String(i.id)===String(g.enhanceId));if(!it||(!transcend&&it.enhance>=15)||(transcend&&!f.canTranscend(it)))return false;const next=transcend?(it.transcend||0)+1:it.enhance+1,cost=transcend?f.transcendCost(it,next):f.enhanceCost(it);if(transcend?state().transcendStone<cost:state().gold<cost)return f.toast('재료가 부족합니다.'),false;
  ...
  operation={id:it.id};g.enhanceBusy=true;$('enhanceResult').textContent='';f.save(false);f.renderEnhance();$('enhanceStage').className='enhance-stage forging';try{f.playForgeSound()}catch(e){}setTimeout(()=>{operation=null;g.enhanceBusy=false;if(!active())return;changed();f.renderEnhance();$('enhanceStage').className='enhance-stage '+(success?'success':'fail');$('enhanceResult').textContent=success?'성공':'실패';window.RinguAudio?.effect(success?'forge-success':'forge-failure');},600);return true;
 }
 ...
 function setupLayout(){
  ...
  document.querySelector('#settingsModal .rank-note')?.remove();document.querySelector('#settingsModal .modal').insertAdjacentHTML('beforeend','<div class="rm-account-tools"><p>현재 로그인: <b id="rmAccountName"></b></p><button id="rmFxToggle">오라 애니메이션 '+(s.remodelFx===false?'OFF':'ON')+'</button><button id="rmLogout">로그아웃</button></div>');$('rmAccountName').textContent=window.RinguSession.account?.username||'계정';$('rmLogout').onclick=()=>window.RinguSession.logout();$('rmFxToggle').onclick=()=>{s.remodelFx=s.remodelFx===false;changed();$('rmFxToggle').textContent='오라 애니메이션 '+(s.remodelFx?'ON':'OFF')};
