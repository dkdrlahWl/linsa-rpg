...
assert.equal(AUDIO_IDS.size,47);
for(const cls of ['warrior','mage','archer','rogue','pirate']){
 ...
 for(const [key,id] of [['attackReady',cls+'-attack'],['ultimateReady',cls+'-skill-1'],['skillReady',cls+'-skill-2'],['thirdReady',cls+'-skill-3'],['dashReady','battle-dash']]){b[key]=100;t.observe(b);assert.equal(ids.at(-1),id);const n=ids.length;t.observe(b);t.observe({...b,[key]:0});t.observe(b);assert.equal(ids.length,n,'prediction replay');}
 ...
 for(let slot=1;slot<=3;slot++)assert.deepEqual(eventAudio({type:'skill',slot},cls),[cls+'-skill-'+slot]);
}
...
console.log('PASS 47 audio ids, all classes/skills, cooldown replay deduplication, cube and enhancement routing');
