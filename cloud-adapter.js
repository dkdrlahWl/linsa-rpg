...
(() => {'use strict';
 ...
 window.fetch=async function(input,init={}){
   ...
   try{
     ...
     if(path==='/api/stone-reward')return response(await rpc('ringu_stone_reward',{p_room:body.room||null},signal));
     if(path==='/api/resource-transfer')return response(await rpc('ringu_resource_transfer',{p_action:body.action||'status',p_recipient:body.recipient||null,p_amount:body.amount??null,p_request_id:body.requestId||null,p_resource:body.resource||'gold'},signal));
