...
try{
 ...
 await db.exec(await readFile(new URL('../supabase/27-stone-reward.sql',import.meta.url),'utf8'));
 const oldRooms=[];
 ...
 for(let i=0;i<6;i++){
  ...
  const claim=async(id)=>{await identity(u);await db.exec('set role authenticated');try{return (await db.query('select public.ringu_stone_reward($1) r',[id])).rows[0].r;}finally{await db.exec('reset role');}};
  const before=Number((await db.query("select state->>'transcendStone' n from ringu_private.accounts where id=$1",[u.id])).rows[0].n||0);
  const credited=await claim(room.id);assert.equal(credited.stoneAward,i+2);assert.equal(credited.balance,before+i+2);assert.equal(credited.claimed,true);assert.equal(credited.roomReward,i+2);
  const retry=await claim(room.id);assert.equal(retry.stoneAward,0);assert.equal(retry.balance,before+i+2);assert.equal(retry.claimed,true);
  const other=await claim(oldRooms[(i+1)%6]);assert.equal(other.roomReward,0);assert.equal(other.claimed,false);
  assert.equal(Number((await db.query("select state->>'transcendStone' n from ringu_private.accounts where id=$1",[u.id])).rows[0].n),before+i+2);
 ...
 console.log('PASS reward claim: actual persisted balances, retry without duplicate, account isolation; all six actual room creation/start/poll/kill/reward paths, existing-room migration, rerun idempotence, unchanged accounts/members/timers/permissions. Synthetic DB only.');
