...
(()=>{'use strict';
 ...
 window.addEventListener('ringu-ready',()=>{
  ...
  modal.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||busy)return;if(b.dataset.tab)selectTab(b.dataset.tab);if(b.dataset.page){page+=Number(b.dataset.page);scrollTop();void refresh();}if(b.dataset.row)void selected(Number(b.dataset.row));});
