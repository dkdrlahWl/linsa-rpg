...
export function itemAttack(it){const lv=Math.max(0,Math.min(15,it.enhance||0)),base=canonicalBase(it),raw=base*(lv<=10?1+lv*.05:1.5+(lv-10)*.2),enhanced=lv?Math.max(Math.ceil(raw),Math.floor(base)+Math.min(lv,10)+Math.max(0,lv-10)*2):Math.floor(base),t=Math.min(3,it.transcend||0),rate=it.slot==='무기'?(it.rarity>=6?.4:.45):(it.rarity>=6?.25:.3);return Math.floor(enhanced*(1+t*rate));}
