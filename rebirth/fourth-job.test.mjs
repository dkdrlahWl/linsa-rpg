...
for(const classId of Object.keys(FOURTH_SKILLS)){
 ...
 assert.equal(count,FOURTH_SKILLS[classId].hits);assert.ok(Math.abs(total-(['warrior','rogue'].includes(classId)?25.2:classId==='pirate'?20:18))<1e-9);
...
console.log('PASS fourth-job skills: warrior/rogue 2520%, pirate 2000%, mage/archer 1800%, 30s cooldown, pulse counts and range, solo/co-op activation, level 150 gate, victory unlock, +10% stats, repeat practice');
