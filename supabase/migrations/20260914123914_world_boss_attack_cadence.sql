-- WB5: one server attack per second, no attacks or catch-up from movement packets.
alter table ringu_private.wb_members add column next_attack_at timestamptz;
  ...
  if p_action='ack' then
   ...
   if v_packet>m.packet then
    ...
    if r.status='running' and now_at>=r.started_at and m.hp>0 then
     ...
     -- A move packet never awards attacks, even in sub-frame gaps between steps.
     if move_count>0 or m.hp<=0 or m.background or was_background or m.seen_at<=now_at-interval '2 seconds' then
      m.next_attack_at:=greatest(now_at,m.still_at)+interval '1 second';
     elsif now_at>=coalesce(m.next_attack_at,now_at+interval '1 second') and now_at>=m.still_at and now_at<r.started_at+interval '7 minutes' then
