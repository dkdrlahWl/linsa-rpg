...
alter table rebirth_private.party_members add column if not exists second_until integer not null default 0;
alter table rebirth_private.party_members add column if not exists second_ready integer not null default 0;
...
create or replace function rebirth_private.party_view(p_user uuid) returns jsonb language sql stable set search_path='' as $$
 ...
 'room',(select jsonb_build_object('id',r.id,'bossId',(r.boss->>'id')::int,'name',r.boss->>'name','status',r.status,'practice',r.practice,'hp',r.hp,'maxHp',r.max_hp,'tick',r.tick,'startedAt',extract(epoch from r.started_at)*1000,'seconds',(r.boss->>'seconds')::int,'isHost',r.host=p_user,'expiresAt',r.expires_at,
 'members',(select coalesce(jsonb_agg(jsonb_build_object('name',m.name,'classId',m.class_id,'hp',m.hp,'maxHp',m.max_hp,'damage',m.damage,'mine',m.player=p_user,'revived',m.revived,'skillReady',m.skill_ready,'secondReady',m.second_ready,'departed',m.departed) order by m.joined_at,m.player),'[]'::jsonb) from rebirth_private.party_members m where m.room=r.id)) from rebirth_private.party_rooms r where r.id=nullif(p.state->>'partyRoom','')::uuid),
 'rooms',(select coalesce(jsonb_agg(q),'[]'::jsonb) from (select r.id,(r.boss->>'id')::int as "bossId",r.boss->>'name' as name,r.practice,(select count(*) from rebirth_private.party_members m where m.room=r.id and not m.departed) as count from rebirth_private.party_rooms r where r.status='waiting' and r.expires_at>now() and coalesce((r.boss->>'raid')::boolean,false) order by r.created_at desc limit 20) q))
    ...
    if eligible then
     ...
     if random()<(r.boss->>'dropChance')::numeric then
      ...
      if jsonb_array_length(st->'items')<300 then st:=jsonb_set(st,'{items}',st->'items'||jsonb_build_array(it));
       ...
       ck:=ck||'|lv'||(it->>'level')||'|q'||(it->>'quality');
