...
begin
 ...
 if not exists(select 1 from rebirth_private.release where enabled) then raise exception 'REBIRTH_MAINTENANCE'; end if;
...
begin
 if not exists(select 1 from rebirth_private.release where enabled and epoch=p_epoch) then raise exception 'REBIRTH_MAINTENANCE'; end if;
...
declare u uuid:=rebirth_private.session_user(); p rebirth_private.players%rowtype; v_seller rebirth_private.players%rowtype; l rebirth_private.listings%rowtype; it jsonb; result jsonb; old rebirth_private.receipts%rowtype; fingerprint jsonb:=jsonb_build_object('market',p_action,'args',p_args); v_price bigint; item_id text; page integer; filter_slot integer; filter_class text;
begin
 ...
 if p_action='sell' then
  ...
  if (it->>'locked')::boolean or (it->>'broken')::boolean or p.state->'pendingCube'->>'id'=item_id or exists(select 1 from jsonb_each_text(p.state->'equipped') where value=item_id) then raise exception 'ITEM_PROTECTED'; end if;
