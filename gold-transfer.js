...
if(g&&session&&!document.getElementById('goldTransferButton')){
 ...
 #goldTransferModal input{width:100%;margin-top:4px}#goldTransferModal label{display:block;margin-top:10px}
 ...
 const button=document.createElement('button');button.id='goldTransferButton';button.type='button';button.innerHTML='<span class="gt-icon" aria-hidden="true">⇄</span><span>송금</span>';button.setAttribute('aria-label','유저에게 골드 송금');document.querySelector('.header-actions')?.prepend(button);
 const modal=document.createElement('dialog');modal.id='goldTransferModal';modal.setAttribute('aria-labelledby','gtTitle');modal.innerHTML='<header><h2 id="gtTitle">골드 송금</h2><button id="gtClose" type="button">닫기</button></header><p>보유 골드 <b id="gtBalance">—</b> G · 수수료 5% 별도 · 소수점 올림</p><section id="gtForm"><button id="gtPickRank" type="button">랭킹에서 받는 사람 선택</button><p id="gtChosen">받는 사람을 선택해 주세요.</p><label>보낼 골드<input id="gtAmount" type="text" inputmode="numeric" placeholder="1 이상의 정수" autocomplete="off"></label></section><div id="gtReview" hidden></div><p id="gtStatus" role="status" aria-live="polite"></p><button id="gtConfirm" type="button" disabled>송금 내용 확인</button><button id="gtBack" type="button" hidden>수정</button>';document.body.append(modal);
 const $=id=>document.getElementById(id);let people=[],chosen=null,balance=0,busy=false,review=null,generation=0,picking=false;
 const errors={INSUFFICIENT_GOLD:'골드가 부족합니다.',TRANSFER_SELF:'자신에게 송금할 수 없습니다.',TRANSFER_RECIPIENT_UNAVAILABLE:'현재 랭킹에서 받을 유저를 찾을 수 없습니다.',TRANSFER_RECIPIENT_LIMIT:'받는 사람의 골드 보유 한도를 초과합니다.',INVALID_ARGUMENTS:'송금 금액을 확인해 주세요.',ECONOMY_NOT_READY:'서버 준비 후 다시 시도해 주세요.'};
 function amount(){const raw=$('gtAmount').value.trim();return /^\d+$/.test(raw)?Number(raw):NaN;}
 function validate(){const n=amount();$('gtConfirm').disabled=busy||!chosen||!Number.isSafeInteger(n)||n<1||n+Math.ceil(n/20)>balance||n>9000000000000;}
 ...
 $('gtConfirm').onclick=async()=>{
  ...
  if(!review){review={recipient:chosen.id,name:chosen.name,identity:identity(chosen),amount:amount()};$('gtForm').hidden=true;$('gtReview').hidden=false;$('gtReview').textContent=review.identity+' 계정에 '+fmt(review.amount)+' G를 보냅니다. 수수료 '+fmt(Math.ceil(review.amount/20))+' G · 총 차감 '+fmt(review.amount+Math.ceil(review.amount/20))+' G';$('gtBack').hidden=false;$('gtConfirm').textContent='확인 · 골드 보내기';$('gtStatus').textContent='받는 사람과 금액을 확인해 주세요.';return;}
  busy=true;validate();$('gtClose').disabled=true;$('gtBack').disabled=true;$('gtStatus').textContent='송금 중…';
  try{const sent=review,result=await session.goldTransferTransaction(sent.recipient,sent.amount);if(!result.ok)throw Error('UNCONFIRMED');chosen=null;$('gtAmount').value='';balance=g.state.gold;$('gtBalance').textContent=fmt(balance);form();$('gtChosen').textContent='받는 사람을 선택해 주세요.';$('gtStatus').textContent=sent.identity+' 계정에 '+fmt(sent.amount)+' G 송금 완료 · 수수료 '+fmt(result.fee??Math.ceil(sent.amount/20))+' G';}
