...
fs.mkdirSync('rebirth/test-artifacts',{recursive:true});
...
try{for(const width of [390,1280]){
 ...
 const it=state.items[0];it.level=200;it.grade=5;it.lines=[{key:'INT',value:12,grade:5},{key:'INT',value:9,grade:4},{key:'attack',value:9,grade:4}];
 ...
 assert.equal(await page.locator('.cube-card').count(),8);
 ...
 await page.screenshot({path:'rebirth/test-artifacts/cube-choice-'+width+'.png'});
 ...
 await page.locator('[data-action="cubeUse"]').click();
 await page.waitForFunction(()=>!document.querySelector('[data-action="cubeUse"]').disabled);
 assert.deepEqual(state.items[0].lines[0],first);
 await page.locator('.cube-probabilities > summary').first().click();
 await page.screenshot({path:'rebirth/test-artifacts/cube-panel-'+width+'.png',fullPage:true});
 assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
 assert.doesNotMatch(await page.locator('dialog').innerText(),/undefined|NaN|품질/);
 assert.deepEqual(errors,[]);
 await page.close();console.log('PASS browser',width,'cube selection, black choice, red immediate, prime lock, probabilities, no overflow/errors');
