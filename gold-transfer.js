...
if(g&&session&&!document.getElementById('goldTransferButton')){
 ...
 const modal=document.createElement('dialog');modal.id='goldTransferModal';modal.setAttribute('aria-labelledby','gtTitle');modal.innerHTML='<header><h2 id="gtTitle">골드 송금</h2><button id="gtClose" type="button">닫기</button></header><p>보유 골드 <b id="gtBalance">—</b> G · 송금액에서 5% 공제 · 우편 수령</p><section id="gtForm"><button id="gtPickRank" type="button">랭킹에서 받는 사람 선택</button><p id="gtChosen">받는 사람을 선택해 주세요.</p><label>보낼 골드<input id="gtAmount" type="text" inputmode="numeric" placeholder="2 이상의 정수" autocomplete="off"></label></section><div id="gtReview" hidden></div><p id="gtStatus" role="status" aria-live="polite"></p><button id="gtConfirm" type="button" disabled>송금 내용 확인</button><button id="gtBack" type="button" hidden>수정</button>';document.body.append(modal);
 const $=id=>document.getElementById(id);let people=[],chosen=null,balance=0,busy=false,review=null,generation=0,picking=false;
 const errors={TRANSFER_TOO_SMALL:'수령액이 1G 이상이 되도록 2G 이상 입력해 주세요.',INSUFFICIENT_GOLD:'골드가 부족합니다.',TRANSFER_SELF:'자신에게 송금할 수 없습니다.',TRANSFER_RECIPIENT_UNAVAILABLE:'현재 랭킹에서 받을 유저를 찾을 수 없습니다.',TRANSFER_RECIPIENT_LIMIT:'받는 사람의 골드 보유 한도를 초과합니다.',INVALID_ARGUMENTS:'송금 금액을 확인해 주세요.',ECONOMY_NOT_READY:'서버 준비 후 다시 시도해 주세요.'};
