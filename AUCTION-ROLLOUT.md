...
- 사용자가 성능 수정 후 정상 플레이를 확인했고 거래 개방을 명시적으로 요청했다.
- 운영 DB 집계에서 서버 경제 동기화 완료 계정 1개 확인. 개방 직전 매물 0개이며 시험 매물을 운영에 삽입하지 않았다.
- `15-open-auction.sql` 적용 성공. 운영 재조회 결과 `auction_open=true`, `economy_ready=true`.
- 경매장 UI는 기존 배포본의 메뉴에서 접근한다. 정수 고정가 / 수수료 0 / 원본 장비 거래이며 코스튬 상점과 분리된다.
- 서버 상태 플래그만 열었다. 운영 유저의 재화·아이템으로 시험 거래를 대신하지 않았다. 실제 유저 사이의 첫 운영 거래는 아직 실행/확인하지 않았다.
- 앞선 격리 DB 두 계정 거래, 실제 PostgreSQL 동시 연결, 브라우저 저장·재접속·원본 옵션 보존 검사가 통과한 상태로 개방했다.
- 용량 오류와 성능 수정도 배포됐다: 복구 사본은 IndexedDB로 보존 이동, 전투 틱 전체 UI 재생성/입력 차단 제거. 2,400장비 시험의 화면 갱신 평균 445ms→1.6ms는 로컬 브라우저 측정값이며 전체 네트워크 지연 측정값이 아니다.

아래 기록은 개방 전 단계별 이력이며, 현재 상태는 이 개방 완료 항목이 우선한다.

### 후속 운영 적용

- 사용자에게 인증 설정의 영향을 설명한 뒤 명시적 승인을 받고 `Verify JWT with legacy secret` OFF를 저장했다. 함수 내부 Auth 사용자 검증과 DB 최신 세션 검증은 유지한다.
- 실제 호스팅 HTTP 검사: 인증 없음 401 LOGIN_REQUIRED / 잘못된 토큰 401 / 허용되지 않은 Origin 403 / 게임 Origin OPTIONS 204.
- SQL 10/11 운영 적용 성공. 장비 카탈로그 대조: 2,394개 중 알 수 없는 장비 0개, 중복 ID 0개.
- 실제 운영 DB에서 이전 리허설 후 ROLLBACK: 계정 8개, 장비 2,394개, 진행 데이터 변경 0개, 원본 장비 속성 변경 0개. 임시 테이블에도 RLS와 권한 회수를 적용했다.
- SQL 14 운영 적용 성공: economy_ready=true, enabled=false, 이전 계정 8개, 등록 장비 2,394개. 기존 서버 저장본과 economy_baselines를 함께 보존한다. 이전 ID/장착 참조만 호환 변환했다.
- 두 브라우저 격리 DB 연결 검사, DB 권한/재전송/원자성 검사 재실행 통과. 별도 PostgreSQL 실제 연결에서 구매/구매 10회, 구매/취소 10회, 동일 요청 8개 동시 처리 통과.
- **남음:** 실제 로그인 사용자의 호스팅 경제 명령 성공 확인과 경매장 enabled=true 개방. 사용자에게 게임 로그인 링크를 전달했다. 인증 없이 로그인 성공을 가정하거나 운영 매물을 가짜로 만들지 않았다.

### 직전 배포 기록 (위 후속 적용 상태가 우선)
