...
(() => {
 ...
 function install(){
  ...
  // Keep tower display data aligned even after legacy balance initialization.
  const towerHp=[7500,12000,18000,27000,37500,51000,67500,87000,105000,127500,157500,187500,217500,247500,285000,322500,360000,472500,585000,697500,810000,922500,1035000,1185000,1335000,1410000,1485000,1560000,1635000,1710000];
  function towerDisplay(){g.towerFloors.forEach((floor,i)=>{if(i>=17){floor.hp=towerHp[i];floor.required=Math.ceil(floor.hp/15);}});}
  towerDisplay();
  const renderTower=f.renderTower;
  f.renderTower=function(...args){towerDisplay();const result=renderTower.apply(this,args);const art=$('towerArt');if(art){art.classList.add('rm-enemy-art');if(!art.style.backgroundImage)art.style.backgroundImage='none';}return result;};
  const towerStyle=document.createElement('style');towerStyle.textContent='body[data-remodel] #towerArt{clip-path:none!important;border-radius:0!important;box-shadow:none!important;animation:none!important;background-color:transparent!important}body[data-remodel] #towerArt::before,body[data-remodel] #towerArt::after{display:none!important}';document.head.append(towerStyle);
  const messages={INVALID_WHEEL_VERSION:'돌림판이 업데이트됐습니다. 새로고침 후 이용해 주세요.',DAILY_WHEEL_LIMIT:'사용 가능한 돌림판 횟수를 모두 사용했습니다. 무료 3회는 한국시간 자정에 초기화됩니다.',WHEEL_BOX_PENDING:'열려 있는 장비 상자의 확인 버튼을 먼저 눌러 주세요.',INVALID_CUBE_TARGET:'이 큐브를 사용할 수 없는 장비입니다.',INSUFFICIENT_JADECUBE:'비취 큐브가 부족합니다.',INSUFFICIENT_SUNCUBE:'태양 큐브가 부족합니다.',REQUEST_TOO_LARGE:'분해 요청이 너무 큽니다. 게임을 새로고침한 뒤 다시 시도해 주세요.',INSUFFICIENT_GOLD:'골드가 부족합니다.',INSUFFICIENT_ESSENCE:'정수가 부족합니다.',INSUFFICIENT_TRANSCENDSTONE:'초월석이 부족합니다.',INSUFFICIENT_PETSTONE:'펫 스톤이 부족합니다.',INSUFFICIENT_TICKET:'뽑기권이 없습니다.',ALREADY_CLAIMED:'이미 받은 보상입니다.',ALREADY_OWNED:'이미 보유하고 있습니다.',ALL_OWNED:'대상 오라를 모두 보유했습니다. 뽑기권은 유지됩니다.',ALL_PETS_MAX:'보유한 모든 펫이 만렙입니다.',ITEM_NOT_OWNED:'현재 보유한 장비가 아닙니다.',ITEM_LOCKED_OR_EQUIPPED:'장착 또는 잠금 해제 후 이용하세요.',ITEM_IN_ESCROW:'경매장에 등록 중인 장비입니다.',PET_NOT_OWNED:'현재 보유한 펫이 아닙니다.',DUNGEON_LOCKED:'입장 횟수 또는 이전 단계 클리어를 확인하세요.',MONSTER_LOCKED:'이전 몬스터를 먼저 처치하세요.',BATTLE_IN_PROGRESS:'현재 전투를 먼저 종료하세요.',COLLECTION_INCOMPLETE:'도감 달성 수가 부족합니다.',SAVE_CONFLICT:'다른 처리가 진행 중입니다. 잠시 후 다시 시도하세요.',INVALID_ARGUMENTS:'입력값을 확인하세요.',INVALID_ENHANCEMENT:'강화·초월 조건을 확인하세요.',ECONOMY_NOT_READY:'서버 업데이트 중입니다. 잠시 후 접속하세요.'};
  ...
  function paint(events=[]){
   ...
   const protection=JSON.stringify([s.useProtect,s.downgradeProtect]),protectionChanged=protection!==lastProtection;lastProtection=protection;
   if((layoutChanged||protectionChanged)&&$('enhanceModal')?.classList.contains('show')){f.renderEnhance();if(forgeBusy){$('enhanceBtn').disabled=true;$('transcendBtn').disabled=true;}}
  ...
  for(const count of [50,100]){const button=document.createElement('button');button.className='draw';button.id='draw'+count;button.innerHTML=count+'회 소환<span id="drawCost'+count+'"></span>';button.onclick=()=>f.drawItems(count);document.querySelector('#summonPanel .draw-buttons')?.append(button);}
  const renderSummon=f.renderSummon;
  f.renderSummon=(...args)=>{const result=renderSummon(...args);for(const count of [50,100])$('drawCost'+count).textContent=(f.summonUnitCost()*count).toLocaleString('ko-KR')+' G';return result;};
  ...
  function sell(items){
   ...
   modal.innerHTML='<section class="modal"><h3>장비 분해</h3><p>장비 <b>'+ids.length.toLocaleString()+'개</b>를 분해합니다.</p><p>장비마다 독립적으로 <b>1%</b> 확률로 정수를 획득합니다.<br>일반 1 · 희귀 2 · 레어 3 · 에픽 4<br>전설 5 · 신화 6 · 타락 7개</p><p>골드는 지급되지 않습니다.<br>분해한 장비는 되돌릴 수 없습니다.</p><button id="rmSellCancel">취소</button><button id="rmSellAccept">확인 · 분해하기</button></section>';
   ...
   $('rmSellAccept').onclick=async()=>{
    ...
    if(event){
     ...
     modal.innerHTML='<section class="modal"><h3>분해 완료</h3><p>장비 <b>'+event.count.toLocaleString()+'개</b> 분해</p><p role="status" style="font-size:20px;color:#e5c0ff">정수 <b>'+Number(event.essence||0).toLocaleString()+'개</b> 획득</p><p>'+(event.essence?'획득한 정수를 보유량에 반영했습니다.':'이번 분해에서는 정수를 얻지 못했습니다. 장비마다 획득 확률은 1%입니다.')+'</p><button id="rmSellDone">확인</button></section>';
  ...
  window.RinguEconomy={command,sync,paint};void import('/linsa-rpg/daily-wheel.js?v=DW4');paint(window.RinguCloud.initialEconomyEvents||[]);window.RinguCloud.initialEconomyEvents=[];
