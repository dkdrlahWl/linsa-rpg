...
(()=>{'use strict';
 ...
 const assets={heroes:new Image(),arena:new Image(),celestialFloor:new Image(),fx:new Image(),tomb:new Image()};assets.heroes.onload=()=>spriteCache.clear();assets.arena.onload=()=>{boardCache=null;};assets.celestialFloor.onload=()=>{boardCache=null;};assets.celestialFloor.src=new URL('art/celestial/auriel-floor-v1.png',document.currentScript.src).href;
 ...
 function shell(){
  ...
  root.innerHTML=`<div class="wb-shell"><header class="wb-header"><div><small>WEEKLY BOSS · 1단계</small><h2>멸겁룡 카르가론</h2></div><time id="wb-time">07:00</time><button id="wb-exit" type="button">나가기</button></header><div id="wb-lobby" class="wb-lobby"></div><div id="wb-battle" class="wb-battle" hidden><canvas id="wb-fx" aria-hidden="true"></canvas><div id="wb-pattern-banner" class="wb-pattern-banner" role="status" aria-live="polite"></div><div class="wb-boss"><div class="wb-bossbar" role="progressbar" aria-label="보스 체력"><i id="wb-boss-fill"></i><span id="wb-boss-hp"></span></div><div id="wb-party" class="wb-party" aria-label="참가자 체력"></div><div id="wb-pattern" class="wb-pattern-label"></div></div><div class="wb-arena-wrap"><canvas id="wb-arena" class="wb-arena" aria-label="8 곱하기 8 전투 타일, WASD 또는 오른쪽 방향키로 이동"></canvas><div id="wb-countdown" class="wb-countdown"></div></div><div class="wb-controls"><div id="wb-contribution" class="wb-contribution" aria-label="참가자 기여도"><strong>기여도</strong><div id="wb-contribution-list"></div></div><div class="wb-control-copy"><strong>이동 중에도 자동공격</strong>초당 최대 1회 공격<br>꾹 누르면 연속 이동<br><br>PC · W A S D</div>${pad('오른손')}</div></div><footer id="wb-footer" class="wb-footer" role="status"></footer></div><div id="wb-overlay" class="wb-overlay" hidden></div>`;
 ...
 function renderLobby(){
  lobbyHtml(`<div class="wb-actions wb-stage-tabs"><button data-action="stage" data-stage="1" aria-pressed="${selectedStage===1}">1단계 · 카르가론</button><button data-action="stage" data-stage="2" aria-pressed="${selectedStage===2}">2단계 · 아우리엘</button></div><div class="wb-cover"><strong>${selectedStage===2?'아우리엘':'카르가론'} <small>· ${selectedStage}단계</small></strong></div>${selectedStage===2?'<p class="wb-copy">체력 8,400,000 · 패턴 피해 2배 · 신규 패턴 11종 · 천공의 심판전</p>':''}<p class="wb-copy">거대한 보스를 마주한 8×8 전장. 이동하면서 공격하고, 보스의 공격을 피하세요.</p><div class="wb-quota">모든 단계 공용 · 이번 주 보상 가능 <b>${remaining} / 3회</b> · 월요일 00:00 초기화 (한국 시간)<br><small>${remaining?'클리어 시 1회 차감 · 보상은 우편함으로 지급':'보상 없이 참가 가능 · 혼자서도 시작 가능'}</small></div>${unlockedAt?'':'<p class="wb-status">잠김 · 랭킹 플레이어 공격력 합계가 최초 8,000 이상이면 영구 해금됩니다.</p>'}<details class="wb-reward-guide"><summary>${selectedStage}단계 클리어 보상 · ${selectedStage===2?'태양 큐브':'비취큐브'} 2개 + 순위별 정수</summary><p>보상 대상 참가자 전원 ${selectedStage===2?'태양 큐브':'비취큐브'} 2개 · 보상은 우편함으로 지급</p><div class="wb-reward-ranks">${Array.from({length:10},(_,i)=>`<span>${i+1}등 <b>정수 ${selectedStage===2?60-i*5:50-i*3}개</b></span>`).join('')}</div><small>동률은 같은 순위 · 예: 공동 1등 ${selectedStage===2?60:50}개, 다음 3등 ${selectedStage===2?50:44}개</small></details><div class="wb-actions"><button class="wb-primary" data-action="create" ${!unlockedAt?'disabled':''}>방 만들기</button><button data-action="refresh" >새로고침</button></div><div class="wb-room-list">${lobbyRooms.length?lobbyRooms.map(r=>`<div class="wb-room-row"><span>${esc(r.host)}의 방<br><small>${r.stage||1}단계 · ${r.count} / 10명</small></span><button data-action="join" data-room="${esc(r.id)}" ${r.count>=10?'disabled':''}>${remaining?'참가':'보상 없이 참가'}</button></div>`).join(''):'<div class="wb-empty">대기 중인 방이 없습니다.<br>방을 만들고 혼자 시작할 수도 있어요.</div>'}</div><details><summary>전투 규칙</summary><ul class="wb-rule-list"><li>플레이어의 최대 체력은 전투 시작 시 본인의 공격력과 같습니다. 보스 피해는 고정 수치입니다.</li><li>이동 중에도 자동공격, 초당 최대 1회 공격. 꾹 누르면 연속 이동합니다.</li><li>PC W 위 · A 왼쪽 · S 아래 · D 오른쪽</li><li>묘비 위에서 3초 대기하면 체력 30%로 부활합니다. 부활 횟수 제한 없음.</li><li>부활 돕는 중에도 공격 가능. 맞아도 게이지는 유지됩니다.</li><li>7분 제한 · 전원 사망 시 실패 · 퇴장/실패 횟수 미차감</li><li>보상 가능 횟수 안에서 클리어한 참가자 전원에게 ${selectedStage===2?'태양 큐브':'비취큐브'} 2개를 지급합니다. 사망·기여도 0도 지급 대상입니다.</li><li>정수는 기여도 1등 ${selectedStage===2?60:50}개부터 순위마다 ${selectedStage===2?5:3}개씩 감소합니다. 동률은 같은 순위입니다. 퇴장자는 제외되며 주 3회 보상 이후에는 보상 없이 참가합니다.</li></ul></details>`);
 ...
 function renderWaiting(){
  ...
  lobbyHtml(`<div class="wb-cover"><strong>출정 준비</strong></div><div class="wb-quota">${remaining?`모든 단계 공용 · 이번 주 보상 ${remaining}회 남음`:'보상 없이 참가 중'} · ${room.members.filter(m=>m.present).length}/10명</div><p class="wb-copy">${host?'모두 준비하면 시작할 수 있습니다. 혼자도 시작 가능합니다.':'준비 완료 후 방장의 시작을 기다려 주세요.'}<br>플레이어의 최대 체력은 본인의 공격력과 같습니다.<br>최대 체력과 외형은 전투 시작 시 고정됩니다.</p><div>${room.members.filter(m=>m.present).map(m=>`<div class="wb-wait-member"><b>${esc(m.name)} ${m.id===room.host?'♛':''} ${m.id===userId?'(나)':''}</b><small>공격력 ${number(m.maxHp)}</small><span class="${m.ready?'wb-ready':'wb-not-ready'}">${m.ready?'준비 완료':'준비 중'}</span></div>`).join('')}</div><div class="wb-actions">${host?`<button class="wb-primary" data-action="start" ${!canStart?'disabled':''}>${room.members.filter(m=>m.present).length===1?'혼자 시작':'전투 시작'}</button>`:`<button class="wb-primary" data-action="ready" >${me.ready?'준비 취소':'준비 완료'}</button>`}</div>`);
  $('wb-footer').textContent=(busy?actionLabels[busyAction]:'')||errorText||'연결이 끊긴 방장은 먼저 참가한 플레이어에게 위임됩니다.';
 }
 let announcedPattern='';
 function announcePattern(){
 ...
 function updateHud(){
  if(!room)return;announcePattern();const me=mine();const t=now();
  const contributionRows=room.members.filter(m=>m.present).sort((a,b)=>b.damage-a.damage).map(m=>'<div class="wb-contribution-row" data-me="'+(m.id===userId)+'"><span>'+esc(m.name)+'</span><b>'+M.contribution(m,room).toFixed(1)+'%</b></div>').join('');
  const list=$('wb-contribution-list');if(list.innerHTML!==contributionRows)list.innerHTML=contributionRows;
  $('wb-contribution').classList.toggle('wb-stale',Date.now()-lastGood>2000);$('wb-contribution').title=Date.now()-lastGood>2000?'연결 복구 중 · 마지막 확인 기록':'참가자 기여도';const rest=M.clamp(Math.ceil((room.startedAt+M.LIMIT_MS-t)/1000),0,420);
 ...
 function board(w){if(boardCache&&boardSize===w)return boardCache;boardSize=w;boardCache=document.createElement('canvas');boardCache.width=boardCache.height=w;const c=boardCache.getContext('2d'),s=w/8;if(room?.stage===2){c.fillStyle='#24313c';c.fillRect(0,0,w,w);if(assets.celestialFloor.complete&&assets.celestialFloor.naturalWidth)c.drawImage(assets.celestialFloor,0,0,w,w);return boardCache;}c.fillStyle='#171615';c.fillRect(0,0,w,w);if(assets.arena.complete&&assets.arena.naturalWidth){c.drawImage(assets.arena,0,0,w,w);return boardCache;}
 ...
 function paint(time){
  ...
  for(const wave of room.waves||[]){
   ...
   for(const tile of wave.tiles){const x=(tile%8+.5)*s,y=(Math.floor(tile/8)+.5)*s;
    if(room.stage===2){
     c.save();c.globalCompositeOperation='source-over';c.globalAlpha=impact>=0?Math.max(0,1-impact/240):1;
     c.fillStyle=impact>=0?'#ff241cf0':'rgba(190,0,24,'+(.66+.16*progress)+')';c.fillRect(x-s*.48,y-s*.48,s*.96,s*.96);
     c.lineWidth=2*dpr;c.strokeStyle=impact>=0?'#fff0b2':'#ff4b55';c.strokeRect(x-s*.46,y-s*.46,s*.92,s*.92);
     c.strokeStyle='#ffccd0';c.lineWidth=1.25*dpr;c.beginPath();c.arc(x,y,s*.30,0,Math.PI*2);c.stroke();
     c.beginPath();c.moveTo(x,y-s*.17);c.lineTo(x,y+s*.06);c.stroke();c.fillStyle='#fff1ed';c.fillRect(x-dpr,y+s*.12,2*dpr,2*dpr);c.restore();
    }else if(assets.fx.complete&&assets.fx.naturalWidth){c.save();c.globalCompositeOperation='screen';c.globalAlpha=impact>=0?Math.max(0,1-impact/240):.65+.35*progress;const col=impact>=0?1:0,row=impact>=0&&wave.damage>=900?1:0,im=assets.fx,sw=im.width/2,sh=im.height/2;c.drawImage(im,col*sw,row*sh,sw,sh,x-s*.5,y-s*.5,s,s);c.restore();}
 ...
 function showResult(){if(!room||$('wb-overlay').dataset.result===room.id)return;$('wb-overlay').dataset.result=room.id;const me=mine(),won=room.status==='won',reward=me?.rewardDetail;
  const rewardText=won?(reward?'<strong>기여도 '+reward.rank+'등 · 클리어 보상</strong><br>정수 <b>'+number(reward.essence)+'개</b> · '+(reward.sunCube?'태양 큐브':'비취큐브')+' <b>'+number(reward.sunCube||reward.jadeCube)+'개</b><br><small>우편함에 도착했습니다. 우편함에서 받아 주세요.</small>':me?.reward==='pending'?'보상 우편 처리 중 · 기록은 보관되어 있습니다.':'이번 전투는 보상 없이 참가했습니다.'):'보상 횟수는 차감되지 않았습니다.';
  const rows=[...room.members].sort((a,b)=>b.damage-a.damage).map(m=>'<div><span>'+(m.rewardDetail?m.rewardDetail.rank+'등 · ':'')+esc(m.name)+(m.present?'':' (퇴장)')+'</span><span>'+number(m.damage)+' · '+M.contribution(m,room).toFixed(1)+'%'+(m.rewardDetail?'<small class="wb-earned">정수 '+number(m.rewardDetail.essence)+' · '+(m.rewardDetail.sunCube?'태양 큐브':'비취큐브')+' '+number(m.rewardDetail.sunCube||m.rewardDetail.jadeCube)+'</small>':'')+'</span></div>').join('');
