...
await db.exec(await readFile(new URL('../supabase/19-summon50.sql',import.meta.url),'utf8'));
await db.exec(await readFile(new URL('../supabase/migrations/20260919164129_summon_one_hundred.sql',import.meta.url),'utf8'));
await db.exec(await readFile(new URL('../supabase/migrations/20260919164129_summon_one_hundred.sql',import.meta.url),'utf8'));
...
try{
 const a=await player('summon100QA');await run(a,'auto',{enabled:false});
 const snap=await rpc('ringu_economy_snapshot');assert.equal(snap.itemIds.length,100);
 await a.setViewportSize({width:390,height:844});await a.locator('.rm-bottom-nav [data-target="summon"]').click();
 assert.match(await a.locator('#draw100').innerText(),/100회 소환/);
 await a.locator('#draw100').click();await a.waitForSelector('#drawResultModal.show #drawResultGrid[data-ringu-result-count="100"]');
 assert.equal(await a.evaluate(()=>RinguCore.state.inventory.length),101);assert.equal(await a.evaluate(()=>RinguCore.state.gold),75000);
 for(const width of [320,390,768]){
  await a.setViewportSize({width,height:844});await a.waitForTimeout(800);
  const result=await a.locator('#drawResultGrid').evaluate(g=>({columns:getComputedStyle(g).gridTemplateColumns.split(' ').length,count:g.children.length,overflow:g.scrollWidth>g.clientWidth,scroll:g.scrollHeight>g.clientHeight}));
  assert.deepEqual(result,{columns:4,count:100,overflow:false,scroll:true});
  await a.screenshot({path:'test-artifacts/summon100-'+width+'.png'});
  await a.locator('#drawResultGrid').evaluate(g=>g.scrollTop=g.scrollHeight);await a.locator('#drawResultGrid>.rm-drop-card').last().scrollIntoViewIfNeeded();assert.ok(await a.locator('#drawResultGrid>.rm-drop-card').last().isVisible());
  await a.locator('#drawResultGrid').evaluate(g=>g.scrollTop=0);
 }
 await a.evaluate(()=>document.querySelector('#drawResultModal').classList.remove('show'));
 await a.setViewportSize({width:390,height:844});
 // Drive the actual queue with two already-discovered copies of the same angel.
 await a.evaluate(()=>RinguRemodel.showServerDraw([0,1].map(i=>({...RinguAngelData.gear[0],id:900+i,isNew:false,enhance:0,transcend:0,optionRolls:[.8,.8]}))));
 for(let i=0;i<2;i++){
  await a.waitForSelector('#angelReveal');await a.waitForTimeout(4500);assert.equal(await a.locator('#angelReveal button').isDisabled(),true);
  await a.keyboard.press('Escape');assert.equal(await a.locator('#angelReveal').count(),1);await a.waitForTimeout(1800);
  assert.equal(await a.locator('#angelReveal button').isEnabled(),true);
  if(!i)await a.screenshot({path:'test-artifacts/angel-seraph-mobile.png'});
  await a.locator('#angelReveal button').click();
 }
 await a.waitForSelector('#drawResultModal.show #drawResultGrid[data-ringu-result-count="2"]');
 assert.deepEqual(errors,[]);console.log('PASS SQL 100 IDs and idempotent migration, server 100-item summon, exact cost, four-column mobile 320/390/768, last item access, repeated angel queue, six-second minimum, confirm button.');
