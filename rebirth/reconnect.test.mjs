...
assert.match(ranking,/async function loadRankings/);
...
for(const stale of [old,'{broken json','{"command":"sync","args":{},"requestId":"stale-sync"}']){
 for(const status of [200,503]){
  ...
  vm.runInContext('let session={access_token:"valid",refresh_token:"valid",expires_at:Date.now()/1000+3600},state=null,busy=false,view="game",connectionLost=false,retryFailures=0,retryAt=0,lastSync=0,rankingUpdated=0,coopRoom=null,coopRooms=[],partyRoom=null,partyRooms=[],combatFrames=[];'+ranking+auth,ctx);
