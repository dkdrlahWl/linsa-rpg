...
for(const cl of CLASSES){
 ...
 const p=power(s);assert.equal(p.stats[cl.stat].total,p.primary);assert.equal(p.stats[cl.stat].fixed,153);assert.equal(p.bonuses[cl.stat],108);assert.equal(p.bonuses.attack,0);assert.equal(p.bonuses.boss,81);
...
console.log('PASS: all five classes, final primary consistency, flat/% totals without set bonuses, combat power, farming exclusions, broken and unequipped exclusions.');
