import {waveStats} from './wave-model.mjs?v=wave-meadow-1';
...
export function waveLobby(state,room,rooms=[]){
 ...
 return `<section class="panel pad wave-lobby"><p class="eyebrow">ENDLESS MEADOW</p><h2>협동 웨이브</h2><p>끝없이 밀려오는 적, 함께 버티는 초원.</p><div class="wave-rules"><span>1~4인 · 레벨 제한 없음</span><span>매 도전 1웨이브부터</span><span>30초마다 다음 웨이브</span><span>전원 사망 / 몬스터 50마리 → 종료</span></div><p class="note">상하좌우 각 5~10마리 + 정예가 첫 10초 동안 몰려옵니다. 10웨이브마다 몬스터 종류 변경, 정예 1마리 추가. 300웨이브까지 30종씩, 301부터 외형 반복.<br>동료 묘비 위에서 움직이지 않고 5초 → 체력 30%로 부활.<br>권장 레벨 = 웨이브 × 2 · 4인 기준 고정 난이도 · 입장 횟수 무제한</p><div class="actions">${button('방 만들기','waveCreate')}${button('목록 새로고침','coopList')}</div><details><summary>누적 보상 · 확률</summary><p>30초를 버틴 웨이브당 5 G + ⌊웨이브 / 10⌋ G.<br>10웨이브 생존: 레드 큐브 1개 5% · 20웨이브 6% · 이후 10단위마다 +1%p, 최대 15%.<br>잠재 해금 주문서는 10·20·30…웨이브마다 개인별 20% 확률로 1개. 큐브와 별도 독립 추첨합니다. 종료·나가기 시 지급. 실패한 웨이브는 미지급.</p></details><p>내 최고 기록 <strong>${fmt(state.waveBest)}웨이브</strong></p><h3>모집 중인 방</h3>${list.length?list.map(r=>`<div class="daily-row"><span>${esc(r.name)}의 초원<small>${r.count} / 4명 · 1웨이브부터 시작</small></span>${button('참가','coopJoin',r.id,r.count>=4)}</div>`).join(''):'<p class="note">새 방을 만들어 혼자서도 출발할 수 있어요.</p>'}</section>`;
}
export function waveHud(host,w){
 ...
 host.querySelector('#tower-enemy-hp').textContent=`남은 몬스터 ${count} / 50 · 처치 ${fmt(w.kills)}`;
 host.querySelector('#tower-enemy-bar').style.width=count/50*100+'%';
 ...
 host.querySelector('#tower-status').textContent=w.pendingOutcome?'종료 판정 확인 중…':me.hp<=0?`동료가 묘비 위에서 5초 대기하면 부활 · ${(me.reviveProgress/10).toFixed(1)} / 5초`:reviving?`동료 부활 중 ${(reviving.reviveProgress/10).toFixed(1)} / 5초 · 움직이면 중단`:count>=40?'위험! 몬스터 50마리가 쌓이면 종료됩니다.':`사방의 적을 처치하세요 · 정예 ${s.eliteCount}마리 등장`;
