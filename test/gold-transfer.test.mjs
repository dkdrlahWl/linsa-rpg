...
try{
 ...
 const request=randomUUID();await call('send',b.id,400,request);const after=await balances();assert.equal(after.find(x=>x.id===a.id).gold,600);assert.equal(after.find(x=>x.id===b.id).gold,1400);
 ...
 assert.equal((await balances()).reduce((n,x)=>n+x.gold,0),3000);
