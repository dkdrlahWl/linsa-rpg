...
for(const classId of Object.keys(FOURTH_SKILLS)){
 ...
 assert.equal(count,FOURTH_SKILLS[classId].hits);assert.ok(Math.abs(total-27)<1e-9);
...
console.log('PASS five fourth-job skills: total 2700%, 30s cooldown, pulse counts and range, solo/co-op activation, level 150 gate, victory unlock, +10% stats, repeat practice');
