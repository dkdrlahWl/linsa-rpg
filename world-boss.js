...
(()=>{'use strict';
 ...
 function updateHud(){
  ...
  $('wb-footer').textContent=(busy?actionLabels[busyAction]:'')||errorText||(Date.now()-lastGood>2000?'연결 복구 중 · 공격 중지':`이동 중에도 자동공격 · 초당 최대 1회 · 부활 ${me?.revived?'1':'0'}/1 · 기여도 ${M.contribution(me,room).toFixed(1)}%${network==='polling'?' · 연결 지연':''}`);
