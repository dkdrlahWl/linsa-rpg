...
# Wire assets. Interaction-time shop templates resolve RinguHUD after deferred scripts load.
...
anchor='<script defer src="/linsa-rpg/black-market.js?v=refresh-BM4"></script>';assert s.count(anchor)==1
s=s.replace(anchor,'<script defer src="/linsa-rpg/hud-ui.js?v=hud-HU1"></script>\n'+anchor,1)
p.write_text(s)
...
old="function topText(costume){ const essence=Math.max(0, Number(g.state.essence)||0); return costume.name+' · '+essence+' 정수 · '+(costume.attackPercent||0)+'% 공격력 보너스'; }";assert s.count(old)==1
s=s.replace(old,"function topText(costume){ const essence=Math.max(0, Number(g.state.essence)||0); return esc(costume.name)+' · '+RinguHUD.essenceCostHTML(essence)+' · '+(costume.attackPercent||0)+'% 공격력 보너스'; }",1)
...
body=r'''try{
 ...
 await context.route('**/*',route=>{const u=new URL(route.request().url());if(u.hostname==='127.0.0.1')return route.continue();if(u.hostname==='ekgihnyojihpearcudtd.supabase.co'){queue=queue.then(async()=>{try{await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(await remote(route.request()))});}catch(e){await route.fulfill({status:400,contentType:'application/json',body:JSON.stringify({error:e.message,message:e.message})});}});return queue;}unexpected.push(u.origin);return route.abort();});
 ...
 const dimensions=[];
 for(const [width,height] of [[320,640],[360,640],[390,844],[412,915],[540,720],[768,800],[1024,768],[1440,1000]]){await page.setViewportSize({width,height});const m=await page.locator('#rmTopHeader').evaluate(el=>{const r=el.getBoundingClientRect(),stats=el.querySelector('#rmCurrencyStats'),summon=el.querySelector('#summonWeaponLevel');return {left:r.left,right:r.right,width:r.width,viewport:innerWidth,overflow:el.scrollWidth>el.clientWidth+1,statsOverflow:stats.scrollWidth>stats.clientWidth+1,summonOverflow:summon.scrollWidth>summon.clientWidth+1};});assert.ok(m.left>=-1&&m.right<=width+1,JSON.stringify(m));assert.equal(m.overflow,false,JSON.stringify(m));assert.equal(m.statsOverflow,false,JSON.stringify(m));assert.equal(m.summonOverflow,false,JSON.stringify(m));dimensions.push({width,height,...m});}
 ...
 await page.evaluate(()=>openShopConfirm({kind:'consumable',type:'protect',name:'강화 하락 방지권',price:10,icon:'🛡️',description:'테스트'}));await page.waitForFunction(()=>document.querySelector('#shopPurchaseConfirm')?.open);
 ...
 assert.deepEqual(errors,[]);assert.deepEqual(unexpected,[]);const fs=await import('node:fs/promises');await fs.mkdir('test-output/HU1',{recursive:true});await page.screenshot({path:'test-output/HU1/hud-390.png',fullPage:false});await fs.writeFile('test-output/HU1/report.json',JSON.stringify({checks:['custom existing-resource icons only','essence is not diamond','ranking/mail/settings controls retained','eight responsive widths no horizontal overflow','summon label not truncated','shared essence confirmation uses icon plus number'],dimensions,productionDataUsed:false},null,2));console.log('PASS HU1 premium HUD, custom existing-resource icons and shared essence icon pricing; mocked accounts only.');
