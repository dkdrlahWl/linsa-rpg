...
try{
 ...
 await db.exec(await readFile(new URL('../supabase/migrations/20260914145700_economy_prepared_snapshot.sql',import.meta.url),'utf8'));
 const snap=async()=> (await db.query('select public.ringu_economy_prepare() s')).rows[0].s;
 for(let i=0;i<8;i++){
  ...
  const fresh=await snap();assert.equal(fresh.enrolled,false);assert.equal(fresh.dailyBoss,null);
 ...
 await start(u,100);await clock('2026-09-14T00:00:00Z');await auth(u);const caught=await snap();assert.ok(caught.dailyBoss);assert.ok(caught.state.mailbox.some(m=>m.id.startsWith('daily-boss:')));assert.equal((await db.query('select count(*)::int n from ringu_private.daily_boss_rewards where account_id=$1',[u.id])).rows[0].n,2);
 ...
 await auth(u);await db.exec('set role authenticated');assert.equal((await snap()).accountId,u.id);await db.exec('reset role;set role anon');await assert.rejects(()=>snap(),/permission denied/);await db.exec('reset role');await db.query("select set_config('test.sid',$1,false)",[randomUUID()]);await assert.rejects(()=>snap(),/SESSION_/);
