...
import {beginThird,stepThird} from './advancement.mjs?v=fourth-rift-1';
import {TOWER_CLASSES,towerFacing,facingVector} from './tower-model.mjs?v=fourth-rift-1';
...
export function advanceCoopRaw(room,user,input,now,frames=[]){
 ...
 for(;w.tick<upto&&w.status==='fighting';w.tick++){
  ...
  for(const m of alive){const c={...TOWER_CLASSES[m.classId],skillScale:{warrior:3.8,mage:4.2,archer:3.5,rogue:4.4,pirate:4}[m.classId],skillCooldown:{warrior:90,mage:110,archer:85,rogue:100,pirate:100}[m.classId]};let [x,y,bits]=w.started+t*100-m.inputAt<1500?m.input:[0,0,0],n=Math.hypot(x,y);if(n>1){x/=n;y/=n;}m.dir=towerFacing(x,y,m.dir??6);m.moving=n>.01;if(m.moving)m.walk=(m.walk||0)+1;let speed=(bits&1)&&c.range>300?17:25;
   if((bits&4)&&t>=m.dashReady){m.dashReady=t+35;m.immune=t+5;m.dashUntil=t+3;const v=facingVector(m.dir??6);m.dx=n?x/Math.hypot(x,y):v.x;m.dy=n?y/Math.hypot(x,y):v.y;}
   ...
   if(t>=m.immune&&t>=m.hurtReady){const hazard=w.hazards.find(h=>t>=h.at&&t<h.end&&dist(m,h)<h.r&&dist(m,h)>=h.inner),mult=hazard?hazard.multiplier:dist(m,e)<105?.6:0;if(mult){const damage=Math.max(1,Math.round(incomingDamage(tier.attack,m.power.defense)*mult*(first?.guard||1)*(second?.guard||1)));m.hp=Math.max(0,m.hp-damage);m.hurtReady=t+5;w.numbers.push({id:++w.serial,value:damage,x:m.x,y:m.y-100,kind:"incoming",start:t,end:t+9});fx("impact",m.x,m.y-40,110);}}
  }
