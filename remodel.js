...
window.installRinguRemodel=function(g){
 ...
 function setupMenuIcons(){
  ...
  const decorate=root=>{const nodes=[...(root.matches?.('button,.panel-title')?[root]:[]),...root.querySelectorAll('button,.panel-title')];nodes.forEach(el=>{if(el.querySelector('canvas,.rm-item-art,.rm-monster-icon')||el.closest('.rm-bottom-nav'))return;const text=el.textContent.trim();const rule=rules.find(([re])=>re.test(text));if(rule&&el.dataset.uiIcon!==rule[1])el.dataset.uiIcon=rule[1];});root.querySelectorAll('.rm-bottom-nav button').forEach(el=>{if(el.closest('.rm-bottom-nav')?.dataset.pv2==='1'||el.querySelector('.pv-icon'))return;const icon={hunt:'⚔️',character:'🧙',summon:'🔮',inventory:'🎒',menu:'🧭'}[el.dataset.target];if(icon&&el.querySelector('span').textContent!==icon)el.querySelector('span').textContent=icon;});};
