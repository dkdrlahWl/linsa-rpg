...
(()=>{'use strict';
 ...
 function gearRank(it,index){const count=window.gearVariantCount,base=window.fixedBaseAtk;if(typeof count!=='function'||typeof base!=='function')return null;const n=Number(count(Number(it.rarity),it.slot)),current=base(it.slot,Number(it.rarity),index);if(!Number.isFinite(current)||!Number.isInteger(n)||n<1||n>10)return null;const rank=1+Array.from({length:n},(_,i)=>base(it.slot,Number(it.rarity),i)).filter(value=>value>current).length;return {rank,roman:['I','II','III','IV','V','VI','VII','VIII','IX','X'][rank-1]};}
 function icon(it,index=0){const grade=Math.max(0,Math.min(6,Number(it.rarity)||0)),row=Math.max(0,slots.indexOf(it.slot)),col=row===0&&grade===6?(index===0?1:0):Math.max(0,Math.min(9,index)),url=gearImage(grade,row,col),rank=gearRank(it,index),label=rank?' · 같은 등급·부위 기본 공격력 '+rank.rank+'위 (강화·초월 제외)':'';return '<span class="rm-item-art" data-gear-cutout="'+grade+':'+row+':'+col+'" role="img" aria-label="'+slots[row]+' 외형'+label+'" title="'+slots[row]+label+'" style="position:relative;background-image:url('+url+');background-size:contain!important;background-position:center!important;background-repeat:no-repeat">'+(rank?'<span class="rm-gear-rank" aria-hidden="true" style="position:absolute!important;left:2px!important;top:2px!important;z-index:2;font:700 10px/1.2 Georgia,serif!important;color:#fff0c4!important;background:rgba(8,11,18,.85);border:1px solid rgba(232,200,132,.45);border-radius:3px;padding:1px 3px;pointer-events:none;letter-spacing:0;white-space:nowrap;text-shadow:0 1px 2px #000">'+rank.roman+'</span>':'')+'</span>';}
