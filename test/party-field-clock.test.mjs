...
try{
 ...
 await db.exec(await readFile(new URL('../supabase/25-party-field-pause.sql',import.meta.url),'utf8'));
 const oldRooms=[];
 for(let i=0;i<6;i++){const r=(await call(users[i],'create',null,i+1)).room;assert.equal(r.maxHp,expected[i]*3);oldRooms.push(r.id);}
 const pausedState=(await db.query('select state from ringu_private.accounts where id=$1',[users[0].id])).rows[0].state;
 assert.equal(pausedState.serverCombat,null);assert.ok(pausedState.serverClock>Date.now()-10000);
