...
try{
 ...
 const p=pages[0];await p.screenshot({path:'test-output/world-boss/lobby-ten.png'});await p.getByRole('button',{name:'전투 시작',exact:true}).click();
 for(const page of pages)await page.locator('#wb-arena').waitFor({state:'visible'});
 await pause(2300);
 await query("update ringu_private.wb_rooms set waves='[]',next_at=now()+interval '1 hour' where id=$1",[roomId]);
 await pause(1200);
 assert.equal((await query('select count(*)::int n from ringu_private.wb_members where room_id=$1 and present',[roomId])).rows[0].n,10);
 ...
 for(const [width,height] of [[320,568],[360,800],[390,844],[412,915],[768,1024],[1280,900]]){
  await p.setViewportSize({width,height});await pause(350);
 ...
 // Winning and confirming returns to the main scene, even for a dead participant.
 await query('update ringu_private.wb_rooms set hp=0 where id=$1',[roomId]);
 await p.getByRole('button',{name:'보상 확인',exact:true}).waitFor({timeout:10000});await p.screenshot({path:'test-output/world-boss/result.png'});
