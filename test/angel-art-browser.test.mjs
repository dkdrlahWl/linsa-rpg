...
try{
 const a=await player('angelArtQA');
 await a.setViewportSize({width:390,height:844});
 const assets=await a.evaluate(async()=>{const rows=[];for(const t of RinguAngelData.gear){const wrap=document.createElement('div');wrap.innerHTML=RinguCore.fn.gearIcon(t);const im=wrap.querySelector('img');im.loading='eager';await im.decode();const cv=document.createElement('canvas');cv.width=im.naturalWidth;cv.height=im.naturalHeight;const c=cv.getContext('2d');c.drawImage(im,0,0);const d=c.getImageData(0,0,cv.width,cv.height).data;let transparent=0;for(let i=3;i<d.length;i+=4)if(d[i]===0)transparent++;rows.push({slot:t.slot,url:im.src,width:im.naturalWidth,transparent:transparent/(d.length/4)});}return rows;});
 for(const r of assets){assert.match(r.url,/-v3.png$/);assert.ok(r.width>=512);assert.ok(r.transparent>.1,'real alpha '+r.slot);}assert.equal(assets.length,7);
 const socket=await a.evaluate(()=>{const s=RinguArt.weaponSocket(RinguAngelData.gear[0],RinguCore.fn.itemIndex);const c=document.createElement('canvas');c.width=s.w;c.height=s.h;const ctx=c.getContext('2d');ctx.drawImage(s.im,0,0);return {w:s.w,h:s.h,alpha:ctx.getImageData(Math.round(s.u*s.w),Math.round(s.v*s.h),1,1).data[3]};});assert.ok(socket.w>=512&&socket.h>=512&&socket.alpha>200);
 await a.evaluate(()=>{const f=RinguCore.fn;document.querySelector('#drawResultGrid').innerHTML=RinguAngelData.gear.map(t=>'<article class="rm-drop-card" style="--drop-color:#eaffff">'+f.gearIcon(t)+'<small>천사</small><strong>'+t.name+'</strong><span>⚔ '+t.baseAtk.toLocaleString()+'</span></article>').join('');document.querySelector('#drawResultModal').classList.add('show');__ringuCompactDrawV10.refresh();});
 await a.waitForTimeout(900);await a.screenshot({path:'test-artifacts/angel-gear-mobile-v3.png'});
 await a.setViewportSize({width:1280,height:900});await a.waitForTimeout(250);await a.screenshot({path:'test-artifacts/angel-gear-desktop-v3.png'});
 await a.evaluate(()=>document.querySelector('#drawResultModal').classList.remove('show'));
 await a.setViewportSize({width:390,height:844});
 await a.evaluate(()=>{window.angelDone=0;RinguAngelReveal({...RinguAngelData.gear[0],enhance:0,optionRolls:[.8,.8]},()=>window.angelDone++);});
 await a.waitForTimeout(1300);assert.equal(await a.locator('#angelReveal.opening .angel-portal').count(),1);await a.screenshot({path:'test-artifacts/angel-opening-v3.png'});
 await a.waitForTimeout(2100);assert.equal(await a.locator('#angelReveal.gather').count(),1);assert.equal(await a.locator('#angelReveal button').isDisabled(),true);
 await a.waitForTimeout(850);assert.equal(await a.locator('#angelReveal.manifest .angel-starburst i').count(),32);await a.screenshot({path:'test-artifacts/angel-manifest-v3.png'});
 await a.waitForTimeout(2050);assert.equal(await a.locator('#angelReveal.ready button').isEnabled(),true);await a.screenshot({path:'test-artifacts/angel-ready-v3.png'});await a.locator('#angelReveal button').click();assert.equal(await a.evaluate(()=>window.angelDone),1);
 await a.emulateMedia({reducedMotion:'reduce'});await a.evaluate(()=>RinguAngelReveal({...RinguAngelData.gear[2],enhance:0,optionRolls:[.8,.8]},()=>{}));await a.waitForTimeout(1000);assert.equal(await a.locator('.angel-starburst').evaluate(x=>getComputedStyle(x).display),'none');await a.evaluate(()=>RinguAngel.stop());
 assert.deepEqual(errors,[]);console.log('PASS: all 7 transparent images load, weapon grip remains opaque, mobile/desktop item cards, timed portal / gather / burst / ready, confirmation and reduced motion.');
