...
for(const r of [3,4,5])for(let i=0;i<10;i++)test(`sets ${r}/${i}: full/partial/mixed and client-server combat stats`,()=>{
 ...
 const result=sets.fromState(s);assert.equal(result.atkPercent,r===3?i+1:bonus);assert.equal(result.critDamage,bonus);assert.equal(result.rows.filter(x=>x.active).length,2);
...
test('epic weapon and armor keep their original attack ranks and effects',()=>{for(const slot of ['무기','투구','갑옷','바지','신발']){const pool=balance.gear.filter(x=>x.rarity===3&&x.slot===slot).sort((a,b)=>b.baseAtk-a.baseAtk);pool.forEach((it,i)=>assert.equal(sets.rank(it).rank,i+1));}for(let i=0;i<10;i++){const items=balance.slots.map(slot=>gear(3,i,slot));assert.equal(sets.evaluate(items).atkPercent,i+1);}});
