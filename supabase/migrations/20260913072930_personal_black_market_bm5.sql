-- BM5: account-owned persistent offers and per-slot consumable stock.
set local lock_timeout='5s';
set local statement_timeout='30s';
select pg_advisory_xact_lock(70909,10);
create table ringu_private.black_market_personal_cycles(
 ...
 offers jsonb not null check(jsonb_array_length(offers)=5),
 primary key(account_id,rotation_id)
);
alter table ringu_private.black_market_personal_cycles enable row level security;
revoke all on ringu_private.black_market_personal_cycles from public,anon,authenticated;
alter table ringu_private.black_market_purchases add column quantity integer not null default 1 check(quantity between 1 and 10);
alter table ringu_private.black_market_purchases drop constraint black_market_purchases_pkey;
create index black_market_stock_usage on ringu_private.black_market_purchases(account_id,rotation_id,slot);
alter table ringu_private.black_market_purchases drop constraint black_market_purchases_price_check;
alter table ringu_private.black_market_purchases add constraint black_market_purchases_price_check check(price between 1 and 100);
...
begin
 ...
 for i in 0..4 loop
  ...
  if r>=6 then
   ...
   offers:=offers||jsonb_build_array(jsonb_build_object('slot',i,'kind','consumable','price',price,'item',template,'limit',case when r=4 then 10 else 5 end));
  else
...
-- Keep the old four-argument client compatible through the quantity default.
drop function public.ringu_black_market(text,text,integer,uuid);
...
begin
 ...
 if kind='consumable' then
  ...
  if it->>'kind' is distinct from 'consumable' or resource is null or resource not in ('transcendStone','downgradeProtect','jadeCube','sunCube')
   ...
   or (resource='downgradeProtect' and price not between 4 and 8)
