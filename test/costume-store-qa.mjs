...
try{
 ...
 await db.query("select public.ringu_account('activate')");await db.query("select public.ringu_account('save',$1::jsonb,0)",[JSON.stringify({essence:599,inventory:[{id:7}],equipped:{weapon:7}})]);
 ...
 const rolledBack=await call();assert.equal(rolledBack.essence,599);assert.equal(rolledBack.revision,1);assert.deepEqual(rolledBack.owned,[]);
 ...
 const request=randomUUID(),first=await call('buy','kael',request,1);assert.equal(first.essence,299);assert.equal(first.attackPercent,5);assert.equal(first.equipped,null);
 const replay=await call('buy','kael',request,1);assert.equal(replay.essence,299);assert.equal(replay.revision,first.revision);
 ...
 await db.exec("update ringu_private.accounts set state=jsonb_set(state,'{essence}','300')");r=await call('buy','serin',randomUUID(),r.revision);assert.equal(r.essence,0);assert.equal(r.attackPercent,10);assert.deepEqual(r.owned,['kael','serin']);
