...
window.installRinguSocial=function(g){
 ...
 function show(p){selected=p;$('playerEquipTitle').textContent=p.name+'의 캐릭터';$('playerEquipStats').textContent='⚔ '+Math.floor(p.power||0).toLocaleString()+' · 탑 '+(p.tower||0)+'층 · '+(names[p.equippedAura]||'오라 없음');
  $('playerEquipGrid').innerHTML='<canvas id="rmRankHero" width="520" height="720" aria-label="캐릭터 외형과 오라"></canvas><div class="rm-rank-equipment">'+(p.equipment||[]).map(v=>{const it={slot:v.s,rarity:v.r,name:v.n,enhance:v.e||0,transcend:v.t||0};return '<div class="rm-rank-item">'+f.gearIcon(it)+'<span>'+esc(v.s)+' · '+f.itemTitleHtml(it)+'<small>⚔ '+Math.floor(v.a||0).toLocaleString()+'</small>'+((v.o?.length&&window.RinguCubes)?'<small>'+RinguCubes.optionBadge({...it,o:v.o})+' '+esc(v.o[0][1])+'%</small>':'')+'</span></div>'}).join('')+(!p.equipment?.length?'<p>아직 장착한 장비가 없습니다.</p>':'')+(p.pet?'<p>동료: '+esc(p.pet.n)+' · Lv.'+p.pet.l+'</p>':'')+'</div>'+RinguGearSets.html(Object.values(mapProfile(p)));$('playerEquipModal').classList.add('show');
 }
 ...
 let last=0;function draw(t){requestAnimationFrame(draw);if(document.hidden||!window.RinguSession.active||t-last<32)return;last=t;
  ...
  const cv=$('rmRankHero');if(selected&&cv&&$('playerEquipModal').classList.contains('show')){const c=cv.getContext('2d');c.clearRect(0,0,520,720);const p=selected;art.hero(c,{costumeId:p.costumeId,playerGender:p.gender,equippedAura:p.equippedAura,hideHelmet:p.hideHelmet,remodelFx:g.state.remodelFx,ownedPets:p.pet?[{uid:'rank-pet',petId:p.pet.id}]:[],equippedPet:p.pet?'rank-pet':null},mapProfile(p),f.itemIndex,t,260,660,570)}
