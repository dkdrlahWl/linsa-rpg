import {CoopMotion} from './coop-motion.mjs?v=monster-portrait-1';
import {TowerInput,projectPlayer} from './tower-input.mjs?v=monster-portrait-1';
import {predictCoopStep} from './coop-model.mjs?v=monster-portrait-1';
import {COOP_TIERS} from './coop-model.mjs?v=monster-portrait-1';
import {towerArena} from './tower-client.mjs?v=monster-portrait-1';
import {TowerRenderer,motionAsset,asset,image,prepareCombatArt} from './tower-renderer.mjs?v=monster-portrait-1';
...
export class CoopController{
 constructor(host,room,send,sound){Object.assign(this,{host,room,send,sound,keys:new Set(),pointers:new Map(),stick:{x:0,y:0},abort:new AbortController(),busy:false,auto:false,disposed:false,motion:new CoopMotion(),lastDraw:0,pendingBits:0,frames:[],sampler:new TowerInput(100),predicted:structuredClone(room),hint:{attack:0,skill:0,dash:0}});this.renderer=new TowerRenderer(host.querySelector('canvas'));this.canvas=host.querySelector('canvas');const opt={signal:this.abort.signal};
  ...
  this.artReady=false;prepareCombatArt(room.members.map(m=>m.classId),COOP_TIERS[room.tier].art).then(()=>{this.artReady=true;});
 ...
 draw(now){
  ...
  const projection={player:me,classId:me.classId,tick:w.tick,dashReady:me.dashReady,dashUntil:me.dashUntil,dashX:me.dx,dashY:me.dy};
  ...
  this.renderer.draw(b,{enemy,projectiles:b.projectiles},player,0,now,input,this.hint);
