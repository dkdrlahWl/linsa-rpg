...
export function drawWaveCreature(g,im,e,time,size){
 ...
 if(!atlas){atlas=build(im,mode);caches.set(im.src,atlas);if(caches.size>8)caches.delete(caches.keys().next().value);}
...
export async function prepareWaveCreature(im,species){try{await im.decode();if(!caches.has(im.src)){await new Promise(resolve=>requestAnimationFrame(resolve));caches.set(im.src,build(im,modes[species%30]));if(caches.size>8)caches.delete(caches.keys().next().value);}}catch{}}
