-- Shared cooperative combat. All trusted writes go through the authenticated Edge Function.
create table if not exists rebirth_private.party_rooms (
 id uuid primary key default gen_random_uuid(), host uuid not null references auth.users(id) on delete cascade,
 boss jsonb not null, practice boolean not null default false, status text not null default 'waiting' check(status in ('waiting','fighting','won','lost','closed')),
 hp bigint not null default 0, max_hp bigint not null default 0, tick integer not null default 0,
 started_at timestamptz, created_at timestamptz not null default now(), expires_at timestamptz not null default now()+interval '10 minutes',
 ended_at timestamptz, claim_key text
);
create table if not exists rebirth_private.party_members (
 room uuid not null references rebirth_private.party_rooms(id) on delete cascade,
 player uuid not null references auth.users(id) on delete cascade,
 name text not null, class_id text not null, stats jsonb not null, hp bigint not null, max_hp bigint not null,
 damage bigint not null default 0, burst_until integer not null default 0, skill_ready integer not null default 0,
 revived boolean not null default false, departed boolean not null default false, joined_at timestamptz not null default now(),
 primary key(room,player)
);
create index if not exists rebirth_party_player on rebirth_private.party_members(player);
create index if not exists rebirth_party_waiting on rebirth_private.party_rooms(created_at desc) where status='waiting';
alter table rebirth_private.party_rooms enable row level security;
alter table rebirth_private.party_members enable row level security;
revoke all on rebirth_private.party_rooms,rebirth_private.party_members from public,anon,authenticated;

create or replace function rebirth_private.party_view(p_user uuid) returns jsonb language sql stable set search_path='' as $$
 select jsonb_build_object('state',p.state,'revision',p.revision,
 'room',(select jsonb_build_object('id',r.id,'bossId',(r.boss->>'id')::int,'name',r.boss->>'name','status',r.status,'practice',r.practice,'hp',r.hp,'maxHp',r.max_hp,'tick',r.tick,'seconds',(r.boss->>'seconds')::int,'isHost',r.host=p_user,'expiresAt',r.expires_at,
 'members',(select coalesce(jsonb_agg(jsonb_build_object('name',m.name,'classId',m.class_id,'hp',m.hp,'maxHp',m.max_hp,'damage',m.damage,'mine',m.player=p_user,'revived',m.revived,'skillReady',m.skill_ready,'departed',m.departed) order by m.joined_at,m.player),'[]'::jsonb) from rebirth_private.party_members m where m.room=r.id)) from rebirth_private.party_rooms r where r.id=nullif(p.state->>'partyRoom','')::uuid),
 'rooms',(select coalesce(jsonb_agg(q),'[]'::jsonb) from (select r.id,(r.boss->>'id')::int as "bossId",r.boss->>'name' as name,r.practice,(select count(*) from rebirth_private.party_members m where m.room=r.id and not m.departed) as count from rebirth_private.party_rooms r where r.status='waiting' and r.expires_at>now() order by r.created_at desc limit 20) q))
 from rebirth_private.players p where p.id=p_user
$$;
revoke all on function rebirth_private.party_view(uuid) from public,anon,authenticated;
...
declare p rebirth_private.players%rowtype; r rebirth_private.party_rooms%rowtype; m rebirth_private.party_members%rowtype;
 ...
 result jsonb; b_id text; won boolean; eligible boolean; claim text; boss_data jsonb; region_id text;
begin
 ...
 if room_id is not null then
   ...
   if upto>r.tick then
    for t in r.tick+1..upto loop
     for m in select * from rebirth_private.party_members where room=room_id and not departed and hp>0 order by joined_at,player loop
      crit:=case when random()<case when t<=m.burst_until and coalesce((m.stats->'skill'->>'crit')::numeric,0)>0 then 1 else (m.stats->>'crit')::numeric end then (m.stats->>'critDamage')::numeric else 1 end;
      burst:=case when t<=m.burst_until then (m.stats->'skill'->>'damage')::numeric else 1 end;
     ...
     if t%3=0 or t%(r.boss->>'patternEvery')::int=0 then
      for m in select * from rebirth_private.party_members where room=room_id and not departed and hp>0 loop
       guard:=case when t<=m.burst_until then (m.stats->'skill'->>'guard')::numeric else 1 end;
  ...
  if r.status in ('won','lost','closed') then
   ...
   won:=r.status='won'; b_id:=r.boss->>'id';region_id:=r.boss->>'region';
   ...
   for m in select * from rebirth_private.party_members where room=room_id and not departed order by player loop
    ...
    eligible:=won and not r.practice and m.damage>0 and st->'bossClaims'->>b_id is distinct from r.claim_key;
    reward:=jsonb_build_object('type','party','bossId',b_id::int,'won',won,'practice',r.practice,'items','[]'::jsonb,'materials',0,'members',summary,'rewarded',eligible);
    if eligible then
 ...
 if p_action in ('create','join') then
  ...
  if p_action='create' then
   ...
   if boss_data is null or (boss_data->>'weekly')::boolean is not true then raise exception 'INVALID_BOSS'; end if;
  ...
  if (p.state->>'level')::int<(r.boss->>'level')::int or not p.state->'cleared' @> jsonb_build_array((r.boss->>'id')::int-1) then raise exception 'PREVIOUS_BOSS_REQUIRED';end if;
  claim:=to_char(date_trunc('week',now() at time zone 'Asia/Seoul'),'YYYY-MM-DD');
  if not r.practice and p.state->'bossClaims'->>(r.boss->>'id')=claim then raise exception 'BOSS_LIMIT';end if;
  ...
  if n<2 then raise exception 'PARTY_MEMBERS_REQUIRED';end if;
  claim:=to_char(date_trunc('week',now() at time zone 'Asia/Seoul'),'YYYY-MM-DD');
  if not r.practice and exists(select 1 from rebirth_private.party_members pm join rebirth_private.players pp on pp.id=pm.player where pm.room=room_id and pp.state->'bossClaims'->>(r.boss->>'id')=claim) then raise exception 'BOSS_LIMIT';end if;
  update rebirth_private.party_rooms set status='fighting',started_at=now(),expires_at=now()+make_interval(secs=>(r.boss->>'seconds')::int),claim_key=claim,hp=floor((r.boss->>'hp')::numeric*(1.7+.6*(n-2))),max_hp=floor((r.boss->>'hp')::numeric*(1.7+.6*(n-2))) where id=room_id;
  ...
  if r.tick<m.skill_ready then raise exception 'SKILL_COOLDOWN';end if;
  update rebirth_private.party_members set burst_until=r.tick+(stats->'skill'->>'seconds')::int,skill_ready=r.tick+30 where room=room_id and player=p_user;
...
begin
 perform rebirth_private.session_user();
 return (select coalesce(jsonb_agg(q),'[]') from (select row_number() over(order by (state->>'level')::int desc,(state->>'xp')::bigint desc,id) as rank,state->>'name' as name,state->>'classId' as "classId",(state->>'level')::int as level,(state->>'xp')::bigint as xp,coalesce((state->>'advancement')::int,0) as advancement from rebirth_private.players where state is not null order by (state->>'level')::int desc,(state->>'xp')::bigint desc,id limit 50) q);
end $$;
