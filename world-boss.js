...
(()=>{'use strict';
 ...
 function accept(data){
  ...
  if(room&&previous?.id===room.id)for(const m of room.members){const h=m.lastHit;if(h&&h.id!==hitSeen.get(m.id)){hitSeen.set(m.id,h.id);if(h.damage>0&&now()-h.at<2500&&(m.id!==userId||(!held.size&&!queuedKey&&now()-lastMove>=M.MOVE_MS)))shots.push({member:m.id,damage:h.damage,crit:!!h.crit,at:now()});}else if(!h){const old=previous.members.find(p=>p.id===m.id);const damage=m.damage-(old?.damage??m.damage);if(damage>0)shots.push({member:m.id,damage,crit:false,at:now()});}}
 ...
 function updateHud(){
  ...
  $('wb-footer').textContent=(busy?actionLabels[busyAction]:'')||errorText||(Date.now()-lastGood>2000?'연결 복구 중 · 공격 중지':`정지 시 자동공격 · 이동 중 중지 · 부활 ${me?.revived?'1':'0'}/1 · 기여도 ${M.contribution(me,room).toFixed(1)}%${network==='polling'?' · 연결 지연':''}`);
 ...
 function move(key){
  ...
  for(let i=shots.length-1;i>=0;i--)if(shots[i].member===userId&&t-shots[i].at<360)shots.splice(i,1);moves.push({...d,seq:++seq,at:t});queuedKey=null;lastMove=t;scheduleSync(40);
 }
 ...
 function paint(time){
  ...
  for(const m of room.members.filter(m=>m.present)){
   ...
   if(hp<=0){if(assets.tomb.complete&&assets.tomb.naturalWidth){c.save();c.globalAlpha=m.revived?.6:1;c.drawImage(assets.tomb,x-s*.45,y-s*.9,s*.9,s*.9);c.restore();}else{c.fillStyle='#807b70';c.fillRect(x-s*.18,y-s*.5,s*.36,s*.5);}
 ...
 function ack(){if(!room){closeLocal();return;}return doAction('ack');}
 ...
 function install(core){if(g)return;g=core;shell();const button=document.createElement('button');button.textContent='월드보스';button.dataset.uiIcon='🐲';button.id='worldBossQuick';button.type='button';button.onclick=openWorldBoss;(document.querySelector('#rmFeatureNav nav')||document.querySelector('.main-quick'))?.append(button);
