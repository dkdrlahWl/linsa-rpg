...
const make=(name,hits,interval,radius,mode,art,description,multiplier=1)=>{const total=(mode==='orbit'?25.2:18)*multiplier/2;return {name,type:'attack',hits,damage:total/hits,interval,range:1100,radius,mode,art,cooldown:15,description:description+' · 총 '+Math.round(total*100)+'% · 재사용 30초'};};
export const FOURTH_SKILLS={
 ...
 pirate:make('대해의 포화',10,8,720,'area',4,'넓은 목표 지역에 10회 함포와 파도 폭발',20/18),
};
