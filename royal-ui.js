...
(()=>{'use strict';
 ...
 function layout(){const header=document.querySelector('.topbar'),boss=document.querySelector('.boss-panel'),nav=document.querySelector('.rm-bottom-nav');for(const el of [header,boss,nav])if(el&&!watched.has(el)){watched.add(el);resize.observe(el);}if(innerWidth>=1100||!header||!boss||!nav)return;const height=Math.max(230,Math.floor(innerHeight-header.getBoundingClientRect().height-boss.getBoundingClientRect().height-nav.getBoundingClientRect().height-28));document.body.style.setProperty('--ru-hunt-height',height+'px');}
 function fit(){layout();document.querySelectorAll('.topbar .stat-value').forEach(el=>{const style=getComputedStyle(el);measure.font=style.font;const size=parseFloat(style.fontSize),width=measure.measureText(el.textContent).width;const fitted=Math.max(12,Math.min(innerWidth>=1100?21:17,Math.floor((el.clientWidth-1)/Math.max(1,width)*size)));el.style.setProperty('font-size',fitted+'px','important');});}
