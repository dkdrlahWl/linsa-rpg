...
try{
 for(let i=0;i<2;i++){
  ...
  if(i===0){await p.setViewportSize({width:1280,height:844});await pause(700);assert.match(await p.locator('#wb-screen').evaluate(x=>getComputedStyle(x).backgroundImage),/dragon-surround-v1/);await p.screenshot({path:'test-artifacts/dragon-surround-1280.png'});await p.setViewportSize({width:390,height:844});await p.locator('[data-action="stage"][data-stage="2"]').click();assert.equal(await p.locator('#wb-screen').getAttribute('data-stage'),'2');assert.match(await p.locator('.wb-reward-guide').innerText(),/태양 큐브 2개/);await p.locator('.wb-reward-guide summary').click();assert.match(await p.locator('.wb-reward-ranks').innerText(),/1등\s+정수 60개/);assert.match(await p.locator('.wb-reward-ranks').innerText(),/10등\s+정수 15개/);await p.getByRole('button',{name:'방 만들기',exact:true}).click();await p.getByRole('button',{name:'혼자 시작',exact:true}).waitFor();roomId=(await query('select id from ringu_private.wb_rooms limit 1')).rows[0].id;}
 ...
 await query('update ringu_private.wb_rooms set hp=0 where id=$1',[roomId]);await p.getByRole('button',{name:'보상 확인',exact:true}).waitFor({timeout:10000});assert.match(await p.locator('#wb-overlay').innerText(),/태양 큐브/);assert.doesNotMatch(await p.locator('#wb-overlay').innerText(),/비취큐브/);await p.getByRole('button',{name:'보상 확인',exact:true}).click();await p.locator('#wb-screen').waitFor({state:'hidden'});assert.deepEqual(errors,[]);console.log('PASS celestial raid browser: stage selection, two-player join/start, 8.4M HP, new pattern labels, movement, 3 viewport sizes, win and exit.');
