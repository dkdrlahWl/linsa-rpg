...
if(g&&session&&!document.getElementById('goldTransferButton')){
 ...
 function validate(){const n=amount();$('gtConfirm').disabled=busy||!chosen||!Number.isSafeInteger(n)||n<1||n>balance||n>9000000000000;}
 ...
 $('gtConfirm').onclick=async()=>{
  ...
  if(!review){review={recipient:chosen.id,name:chosen.name,identity:identity(chosen),amount:amount()};$('gtForm').hidden=true;$('gtReview').hidden=false;$('gtReview').textContent=review.identity+' 계정에 '+fmt(review.amount)+' G를 보냅니다.';$('gtBack').hidden=false;$('gtConfirm').textContent='확인 · 골드 보내기';$('gtStatus').textContent='받는 사람과 금액을 확인해 주세요.';return;}
