...
window.installRinguWorld2=function(g){
 ...
 g.rarityNames[1]='고급';g.rarityNames[2]='희귀';g.rarityColors[6]='#8A235F';
 ...
 f.itemTitleHtml=(it,progress=true)=>{const t=template(it);if(!t)return oldTitle(it,progress);return '<span class="w2-rank" title="타락 계열 '+t.rank+'위">'+t.roman+'</span> <span class="cube-item-name">'+esc((it.locked?'[잠금] ':'')+it.name)+'</span>'+(progress?' <span class="ringu-plus">+'+(it.enhance||0)+'</span> <span class="transcend-stars">'+(it.transcend?'초월 '+it.transcend:'')+'</span>':'');};
 ...
 f.renderSummon=()=>{const v=f.activeSummon(),p=progress(v),cost=f.summonUnitCost();$('summonLevel').textContent=({weapon:'무기',armor:'방어구',accessory:'장신구'}[g.drawGroup])+' 소환 Lv.'+v.level+' · 1회 '+fmt(cost)+' G';$('summonExp').textContent=(p.need?fmt(p.have)+' / '+fmt(p.need)+' EXP':'MAX')+(p.note?' · '+p.note:'');$('expFill').style.width=p.percent+'%';for(const n of [1,5,10,50])if($('drawCost'+n))$('drawCost'+n).textContent=fmt(cost*n)+' G';};
 f.openRates=()=>{$('rateTable').innerHTML='<p>무기·방어구·장신구 경험치는 각각 독립적으로 증가합니다.</p><div class="w2-rate-scroll"><table class="rate-table"><thead><tr><th>레벨</th>'+g.rarityNames.map(n=>'<th>'+esc(n)+'</th>').join('')+'</tr></thead><tbody>'+g.rates.map((r,i)=>'<tr class="'+(i+1===f.activeSummon().level?'current':'')+'"><th>'+ (i+1)+'</th>'+Array.from({length:7},(_,j)=>'<td>'+(r[j]||0)+'%</td>').join('')+'</tr>').join('')+'</tbody></table></div><p>타락 계열: I 1/15 · II 2/15 · III 3/15 · IV 4/15 · V 5/15. 무기는 확정, 방어구 4부위·장신구 2부위는 각각 균등 확률입니다.</p><p>Lv.16→17: 해당 종류 10,000회 + 라지엘 처치<br>Lv.17→18: 해당 종류 20,000회 + 네비로스 처치<br>지역 조건이 부족하면 경험치는 요구량에서 멈추며 초과분은 이월되지 않습니다.</p>';$('rateModal').classList.add('show');};
 ...
 f.renderBosses=()=>{const s=g.state,unlocked=s.monsterUnlockStep||0;$('world2Entrance')?.remove();$('regionTabs').innerHTML=g.bossRegions.map((r,i)=>{const locked=i*6>unlocked||(i>=6&&!s.world2Unlocked);return '<button class="region-tab '+(i===s.regionIndex?'active ':'')+(locked?'locked':'')+'" onclick="selectRegion('+i+')" '+(locked?'disabled':'')+'>'+(i>=6?'<img src="'+base+'region-'+(i-5)+'.webp" alt="" loading="lazy">':'')+'<span>'+(i>=6?'2세계 · ':'')+esc(r.name)+(locked?' · 잠금':'')+'</span></button>';}).join('');$('bossList').innerHTML=f.currentBosses().map((b,i)=>{const locked=s.regionIndex*6+i>unlocked;return '<button class="boss-btn '+(i===s.bossIndex?'active ':'')+(locked?'locked':'')+'" onclick="selectBoss('+i+')" '+(locked?'disabled':'')+'>'+(s.regionIndex>=6?'<img class="rm-monster-icon" src="'+base+b.art+'.webp" alt="" loading="lazy">':'<img class="rm-monster-icon" src="'+oldBossPreview(s.regionIndex*6+i)+'" alt="">')+'<strong>'+esc(b.name)+(locked?' · 잠금':'')+'</strong><span>HP '+fmt(b.hp)+' · '+fmt(b.reward)+' G</span></button>';}).join('');};
